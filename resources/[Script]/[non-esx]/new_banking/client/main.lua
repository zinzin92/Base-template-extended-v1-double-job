		...
		for i = 1, #atms, 1 do
			if #(atms[i] - plyCoords) <= 2.0 then
				...
				if IsControlJustReleased(0, 38) then
					...
					_TriggerServerEvent('bank:balance')
				end
			end
		end
...
RegisterNetEvent('bank:refreshbalance')
AddEventHandler('bank:refreshbalance', function(balance)
	SendNUIMessage({
		type = 'balanceHUD',
		balance = balance,
...
RegisterNUICallback('deposit', function(data)
	_TriggerServerEvent('bank:deposit', tonumber(data.amount))
	_TriggerServerEvent('bank:balance')
end)

RegisterNUICallback('withdrawl', function(data)
	_TriggerServerEvent('bank:withdraw', tonumber(data.amountw))
	_TriggerServerEvent('bank:balance')
end)
...
RegisterNUICallback('transfer', function(data)
	_TriggerServerEvent('bank:transfer', data.to, data.amountt)
	_TriggerServerEvent('bank:balance')
end)
