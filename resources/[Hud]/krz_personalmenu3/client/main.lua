...
Citizen.CreateThread(function()
	...
	RMenu.Settings('personal', 'inventory', 'ButtonLabel', ('%s [%s / %s]'):format(_U('inventory_title'), GetCurrentWeight() + 0.0, ESX.PlayerData.maxWeight + 0.0))
	...
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		if type == 'inventory' then
			RageUI.Button(_U('inventory_use_button'), "", {}, true, function(Hovered, Active, Selected)
				if (Selected) then
					_TriggerServerEvent('esx:useItem', PersonalMenu.ItemSelected.name)
				end
			end)
					...
					if closestDistance ~= -1 and closestDistance <= 3 then
						...
						if IsPedOnFoot(closestPed) then
							if PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name] ~= nil and PersonalMenu.ItemSelected.count > 0 then
								_TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_standard', PersonalMenu.ItemSelected.name, PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name])
								RageUI.CloseAll()
							else
					...
					if PersonalMenu.ItemSelected.canRemove then
						if IsPedOnFoot(plyPed) then
							if PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name] ~= nil then
								_TriggerServerEvent('esx:dropInventoryItem', 'item_standard', PersonalMenu.ItemSelected.name, PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name])
								RageUI.CloseAll()
							else
								ESX.ShowNotification(_U('amount_invalid'))
							end
		...
		elseif type == 'loadout' then
			if HasPedGotWeapon(plyPed, PersonalMenu.ItemSelected.hash, false) then
				RageUI.Button(_U('loadout_give_button'), "", {}, true, function(Hovered, Active, Selected)
					if (Selected) then
						...
						if closestDistance ~= -1 and closestDistance <= 3 then
							...
							if IsPedOnFoot(closestPed) then
								_TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_weapon', PersonalMenu.ItemSelected.name, nil)
								RageUI.CloseAll()
							else
								ESX.ShowNotification(_U('in_vehicle_give', PersonalMenu.ItemSelected.label))
							end
								...
								if plyAmmo > 0 then
									...
									if post then
										if plyAmmo >= quantity then
											_TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_ammo', PersonalMenu.ItemSelected.name, quantity)
											RageUI.CloseAll()
										else
				...
				RageUI.Button(_U('loadout_drop_button'), "", {RightBadge = RageUI.BadgeStyle.Alert}, true, function(Hovered, Active, Selected)
					if (Selected) then
						if IsPedOnFoot(plyPed) then
							_TriggerServerEvent('esx:dropInventoryItem', 'item_weapon', PersonalMenu.ItemSelected.name)
							RageUI.CloseAll()
						else
							ESX.ShowNotification(_U('in_vehicle_drop', PersonalMenu.ItemSelected.label))
						end
		...
		if Config.JSFourIDCard then
			...
			RageUI.Button(_U('wallet_show_firearms_button'), nil, {}, true, function(Hovered, Active, Selected)
				if (Selected) then
					...
					if closestDistance ~= -1 and closestDistance <= 3.0 then
						_TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer), 'weapon')
					else
						ESX.ShowNotification(_U('players_nearby'))
					end
				end
			...
			RageUI.Button(_U('wallet_check_firearms_button'), nil, {}, true, function(Hovered, Active, Selected)
				if (Selected) then
					_TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), 'weapon')
				end
			end)
		end
	end)
...
function RenderOtherMenu()
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		...
		RageUI.Checkbox('Interface GPS', nil, Player.minimap, {}, function(Hovered, Active, Selected)
			if (Selected) then
