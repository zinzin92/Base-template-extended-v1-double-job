			...
			if not alreadyInTable then
				...
				table.insert(employees, {
					...
					job2 = {
						...
						grade_name = Jobs[society].grades[tostring(row.jo2b_grade)].name,
		...
		if salary <= Config.MaxSalary then
			MySQL.Async.execute('UPDATE job_grades SET salary = @salary WHERE job_name = @job_name AND grade = @grade', {
				...
				for i=1, #xPlayers, 1 do
					...
					if xTarget.job2.name == job2 and xTarget.job2.grade == grade2 then
			...
			print(('esx_society: %s attempted to setJob2Salary over config limit!'):format(xPlayer.identifier))
			cb()
		end
	else
		print(('esx_society: %s attempted to setJob2Salary'):format(xPlayer.identifier))
		cb()
	end
end)
