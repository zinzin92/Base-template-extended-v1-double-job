...
function DisableSociety2MoneyHUDElement()
	...
	TriggerEvent('esx_society:toggleSociety2Hud', false)
end
...
function OpenBossMenu(society, close, options)
	...
	if options.withdraw and ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job2.grade_name == 'boss'  then
		table.insert(elements, {label = _U('withdraw_society_money'), value = 'withdraw_society_money'})
	end

	if options.deposit and ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job2.grade_name == 'boss' then
		table.insert(elements, {label = _U('deposit_society_money'), value = 'deposit_money'})
	end

	if options.wash and ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job2.grade_name == 'boss' then
		table.insert(elements, {label = _U('wash_money'), value = 'wash_money'})
	end

	if options.employees and ESX.PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = _U('employee_management'), value = 'manage_employees'})
	end
	
	 if options.employees and ESX.PlayerData.job2.grade_name == 'boss' then
         table.insert(elements, {label = _U('secondary_employee_management'), value = 'secondary_manage_employees'})
	 end
	
	 --if options.employees and PlayerData.job3.grade_name == 'boss' then
		 --table.insert(elements, {label = _U('employee_management'), value = 'triple_manage_employees'})
	 --end

	 if options.grades and ESX.PlayerData.job.grade_name == 'boss' then
		 table.insert(elements, {label = _U('salary_management'), value = 'manage_grades'})
	 end
	 
	 if options.grades and ESX.PlayerData.job2.grade_name == 'boss' then
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'manage_grades2' then
...
function OpenManageGradesMenu2(society)
  ...
  ESX.TriggerServerCallback('esx_society:getJob2', function(job2)
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'manage_grades2_' .. society,
      {
        title    = _U('salary_management2'),
        elements = elements
      },
      function(data, menu)
